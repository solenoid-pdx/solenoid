    ...
    if (request.method == "POST"):
        ...
        if form.is_valid():
            ...
            if data['xGraph'] == 'voltage':
                x = 'Voltage' 
                for k,v in solenoid_range(data['voltage'],data['length'], data['r0'], data['ra'], data['awg'], data['x'], data['force'], data['relative_permeability'], data['xGraph'], 0.0, 15.0, 1.0):
                    labels.append(k)
                ...
                x = 'r0'
                for k,v in solenoid_range(data['voltage'],data['length'], data['r0'], data['ra'], data['awg'], data['x'], data['force'], data['relative_permeability'], data['xGraph'], 1.0, 5.0, 0.1):
                ...
                for k,v in solenoid_range(data['voltage'],data['length'], data['r0'], data['ra'], data['awg'], data['x'], data['force'], data['relative_permeability'], data['xGraph'], 0.0, length, 1.0):
                  labels.append(k)
                  graph.append(round(v,sigFigs))  
                  
            elif data['xGraph'] == 'awg':
                x = 'American Wire Gauge'
                for k,v in solenoid_range(data['voltage'],data['length'], data['r0'], data['ra'], data['awg'], data['x'], data['force'], data['relative_permeability'], "gauge", 1.0, 5.0, 0.1):
                  labels.append(k)
                  graph.append(round(v,sigFigs))  

            elif data['xGraph'] == 'force':
                x = 'Force'
                for k,v in solenoid_range(data['voltage'],data['length'], data['r0'], data['ra'], data['awg'], data['x'], data['force'], data['relative_permeability'], data['xGraph'], 1.0, 50.0, 1.0):
                  labels.append(k)
                  graph.append(round(v,sigFigs))  
                ...
                for volts in range(0, 51):
                    labels.append(volts)
                    graph.append(str(round(solenoid_solve(volts, data['length'], data['r0'], data['ra'], data['awg'], data['x'], data['force'], data['relative_permeability']),sigFigs)))
