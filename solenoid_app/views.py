...
# Receives the ajax request from the frontend, then asks for data from solve.py
        ...
        if form.is_valid():
            ...
            if data['x_graph'] == 'voltage':
                ...
                for k, v in solenoid_range(data['voltage'], data['length'], data['r0'], data['ra'], data['awg'],
                                           data['x'], data['force'], data['relative_permeability'], output_unit,
                                           data['x_graph'], idv_unit, data['x_start'], data['x_end'], data['x_step']):
                    labels.append(k)
                ...
                for k, v in solenoid_range(data['voltage'], data['length'], data['r0'], data['ra'], data['awg'],
                                           data['x'], data['force'], data['relative_permeability'], output_unit,
                                           data['x_graph'], idv_unit, data['x_start'], length, data['x_step']):
                    labels.append(k)
                    graph.append(round(v, sig_figs))
                ...
                for k, v in solenoid_range(data['voltage'], data['length'], data['r0'], data['ra'], data['awg'],
                                           data['x'], data['force'], data['relative_permeability'], output_unit,
                                           "relative_permeability", idv_unit, data['x_start'], data['x_end'],
                                           data['x_step']):
                    labels.append(k)
                    graph.append(round(v, sig_figs))
    else:
